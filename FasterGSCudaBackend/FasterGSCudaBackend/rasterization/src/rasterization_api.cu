...
faster_gs::rasterization::forward_wrapper_dr(
    const torch::Tensor& means,
    const torch::Tensor& scales,
    const torch::Tensor& rotations,
    const torch::Tensor& opacities,
    ...
    const bool proper_antialiasing)
{
    ...
    CHECK_INPUT(config::debug, sh_coefficients_0, "sh_coefficients_0");
    CHECK_INPUT(config::debug, sh_coefficients_rest, "sh_coefficients_rest");
    CHECK_INPUT(config::debug, features, "features");
    CHECK_INPUT(config::debug, sh_rotation, "sh_rotation");
    ...
    const torch::TensorOptions byte_options = torch::TensorOptions().dtype(torch::kByte).device(torch::kCUDA);
    torch::Tensor image = torch::empty({3, height, width}, float_options);
    torch::Tensor feature_map = torch::empty({4, height, width}, float_options);
    torch::Tensor primitive_buffers = torch::empty({0}, byte_options);
    torch::Tensor tile_buffers = torch::empty({0}, byte_options);
    torch::Tensor instance_buffers = torch::empty({0}, byte_options);
    torch::Tensor bucket_buffers = torch::empty({0}, byte_options);
    ...
    auto [n_instances, n_buckets, instance_primitive_indices_selector] = forward(
        ...
        far_plane,
        proper_antialiasing,
        reinterpret_cast<float4*>(features.contiguous().data_ptr<float>()),
...
faster_gs::rasterization::backward_wrapper_dr(
    torch::Tensor& densification_info,
    const torch::Tensor& grad_image,
    const torch::Tensor& grad_feature_map,
    ...
    const int instance_primitive_indices_selector)
{
    ...
    torch::Tensor grad_features = torch::zeros({n_primitives, 4}, float_options);
    ...
    backward(
        ...
        proper_antialiasing,
        reinterpret_cast<float4*>(features.contiguous().data_ptr<float>()),
        grad_feature_map.contiguous().data_ptr<float>(),
        feature_map.data_ptr<float>(),
        reinterpret_cast<float4*>(grad_features.data_ptr<float>())
    );

    return {grad_means, grad_scales, grad_rotations, grad_opacities, grad_sh_coefficients_0, grad_sh_coefficients_rest, grad_features};
}
